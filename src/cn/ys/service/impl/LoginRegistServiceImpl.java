...
public class LoginRegistServiceImpl implements LoginRegistService {
	...
	/* (non-Javadoc)
	 * @see cn.ys.service.impl.LoginRegistService#login(java.lang.String, java.lang.String)
	 */
	@Override
	public int login(String username, String password) {
		int status;
		// 检测用户是否存在
		if (!udao.userExist(username)) {
			// 用户不存在
			status = 3;
		} else {
			// 密码校验
			if (udao.isPasswordCorrect(username, password)) {
				// 密码正确
				status = 1;
			} else {
				status = 2;
	...
	public int registUser(User user) {
		int status;
		// 检测用户是否存在
		if (udao.userExist(user.getUsername())) {
			// 用户已存在
			status = 2;
		} else {
			// 用户不存在，添加用户进入数据库
